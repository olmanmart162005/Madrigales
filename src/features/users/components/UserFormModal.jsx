...
import { z } from 'zod'
import { Eye, EyeOff, User, Mail, Lock, Phone, Shield, CheckCircle2 } from 'lucide-react'
...
const userCreateSchema = z
  .object({
    ...
    username: z
      ...
      .regex(/^[a-zA-Z0-9_.-]+$/, 'Solo letras, números, puntos o guiones'),
...
export default function UserFormModal({ isOpen, onClose, userToEdit, onSuccess }) {
  ...
  // Auto-sugerir correo al escribir username en modo creación
  const handleUsernameChange = (e) => {
    const val = e.target.value.toLowerCase().replace(/[^a-z0-9_.-]/g, '')
    setValue('username', val, { shouldValidate: true })
    if (!isEditing && val) {
      setValue('email', `${val}@madrigales.com`, { shouldValidate: true })
    }
  }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
        ...
        <div>
          <label className="label text-xs">
            Nombre de Usuario para Iniciar Sesión * <span className="text-purple-600 font-normal">(sin espacios)</span>
          </label>
          <div className="relative">
            <span className="text-gray-400 font-bold absolute left-3 top-1/2 -translate-y-1/2 text-xs">@</span>
            <input
              {...register('username')}
        ...
        {!isEditing && (
          <div>
            <label className="label text-xs">Correo Electrónico de Autenticación *</label>
            <div className="relative">
              <Mail className="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" />
              <input
                {...register('email')}
                type="email"
                placeholder="suri@madrigales.com"
                className="input pl-9 text-xs"
              />
            </div>
            {errors.email && (
        ...
        {!isEditing && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="label text-xs">Contraseña Inicial *</label>
              <div className="relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="label text-xs">Teléfono (Opcional)</label>
            <div className="relative">
          ...
          <div>
            <label className="label text-xs">Rol del Usuario *</label>
            <div className="relative">
